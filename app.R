...
#########################################################################
# Things to do/add
# - need to add stations by region or watershed. Need watershed information
# - need to add widgets to dump out results
# - Long term trend for timing of min/max - not sure how useful this will be
# - figure out how to deal with "error" messages when system is intializing.
# - convert to fasstr function when these are fixed to deal with water year and partial months
# - initialize sliders for years with some decent values if the input station is null etc. This will avoid the error messages when starting
# - fix diverging scale in trendM map. Currently not centered at 0 slope.
# - DONE deal with log(0) for some statistics in flow when stream dries up. Current bombs? or comes back with NA. Replace by log(x+offset)

# - Analysis methods
# - currently used LS slope in graphs and maps. Give option to show the MK sen slope
# - need to deal with autocorrelation as needed
# - need to implement other trending methods such as semi-parametric regression
# - need standalone function to create leaflet plots so that I can use them outside of the shiny app like trend_stats
# 
###########################################################################
...
myOptions <- NULL
myOptions$debug        <- TRUE  # print debugging information
myOptions$trendM.min.n <- 5 # minimum number of years to compute a trend
  ...
  dashboardBody(
    fluidPage(
      tabBox(width = 12,
             ...
             ###########################################################################
             # Long term trends. For annual data, select the starting month and the time period in months to select each year
             tabPanel("Long-term Trends",
                fluidRow(
                  column(2,
                    radioButtons("trendType", "Types of values",
                      c("Annual values"  = "trendTypeValue",
                        "Annual timings" = "trendTypeTiming",
                        "Monthly"   = "trendFreqMonthly")),
                  ),
                  column(2,
                    conditionalPanel(condition="input.trendType == 'trendTypeValue' || input.trendType == 'trendTypeTiming'",
                               selectInput("trendAnnualMonthStart","Starting Month",choices=1:12, selected=1))),
                  column(2,
                    conditionalPanel(condition="input.trendType == 'trendTypeValue' || input.trendType == 'trendTypeTiming'",
                               selectInput("trendAnnualNumMonth"  ,"# of Months in year",choices=1:12, selected=12))),
                  column(width=6, uiOutput("trendYears"))
                ),
                hr(),
                fluidRow(
                  tabBox(width = 12,
                    tabPanel("Graph",
                      fluidRow(column(width=12)),
                      fluidRow(column(width=12,plotlyOutput('trendPlot'))),
                      h5("* Mean values produced only for years of complete data"),
                      br(),br(),
                    )  # end of tabPanel
                  )    # end of tabBox
                ),
                # graph options and the statistics
                fluidRow(
                  tabBox(width=12,
                    tabPanel(title = "Graph Options",status = "primary",
                       fluidRow(
                         column(width=3,hr(),
                            uiOutput("trendParam"), 
                            shiny::checkboxInput("trendlog", 
                                      label = "Log scale on primary Y-axis", value= TRUE),
                            shiny::checkboxInput("trendShowFits", 
                                      label = "Show the fitted LS line", value= TRUE)
                         ),
                         column(width=3,hr(),uiOutput("trendStat"))
                       )# end of fluidrow
                    ), # end of box
                    tabPanel("Trend statistics",
                      fluidRow(column(width=9)),
                      conditionalPanel( condition="input.trendlog", 
                                        p("Slope computed on log() scale",style = "color:red") ),
                      DT::dataTableOutput("trendSummaryFit")
                    ) # end of tabPanel
                  )  # end of tabBox
                ) # end of fluidrow
             ...
             ###########################################################################
             # Long term trends - multiple. For annual data, select the starting month and the time period in months to select each year
             tabPanel("Long-term Trends - Multiple Stations",
                fluidRow(
                  column(2,
                    radioButtons("trendMType", "Types of values",
                      c("Annual values"  = "trendMTypeValue",
                        "Annual timings" = "trendMTypeTiming",
                        "Monthly"   = "trendMFreqMonthly")),
                  ),
                  column(2,
                    conditionalPanel(condition="input.trendMType == 'trendMTypeValue' || input.trendMType == 'trendMTypeTiming'",
                               selectInput("trendMAnnualMonthStart","Starting Month",choices=1:12, selected=1))),
                  column(2,
                    conditionalPanel(condition="input.trendMType == 'trendMTypeValue' || input.trendMType == 'trendMTypeTiming'",
                               selectInput("trendMAnnualNumMonth"  ,"# of Months in year",choices=1:12, selected=12))),
                  column(width=6, uiOutput("trendMYears"))
                ),
                hr(),
                fluidRow(
                  tabBox(width = 12,
                    tabPanel("Graph",
                      fluidRow(column(width=12)),
                      fluidRow(column(width=12,plotlyOutput('trendMPlot'))),
                      h5("* Mean values produced only for years of complete data"),
                      br(),br(),
                    )  # end of tabPanel
                  )    # end of tabBox
                ...
                fluidRow(
                  tabBox(width=12,
                    tabPanel(title = "Graph Options",status = "primary",
                       fluidRow(
                         column(width=3,hr(),
                            uiOutput("trendMParam"), 
                            shiny::checkboxInput("trendMlog", 
                                      label = "Log scale on primary Y-axis", value= TRUE),
                            shiny::checkboxInput("trendMShowFits", 
                                      label = "Show the fitted LS line", value= TRUE)
                         ),
                         column(width=3,hr(),uiOutput("trendMStat"))
                       )# end of fluidrow
                    ), # end of box
                    tabPanel(title = "Add/remove stations",
                       fluidRow(
                          column(width=3,hr(),uiOutput("trendMAddStation")),
                          column(width=3,hr(),uiOutput("trendMAddStationByDistance1"),
                                              uiOutput("trendMAddStationByDistance2"))
                       )# end of fluidrow
                    ), # end of tabPanel
                    tabPanel("Trend statistics",
                      fluidRow(column(width=9)),
                      conditionalPanel( condition="input.trendMlog", 
                                        p("Slope computed on log() scale",style = "color:red") ),
                      DT::dataTableOutput("trendMSummaryFit")
                    ), # end of tabPanel
                    tabPanel("Map",
                      br(),
                      fluidPage(
                        column(width=12,
                          # tags$style(type = "text/css", "#map {height: calc(100vh - 170px) !important;}"),
                          leafletOutput("trendMmap",width="100%",height="650px")
                        )
                     )     # end of fluidPage
                    )      # end of tabPane,
                  )  # end of tabBox
...
server <- function(input, output, session) {
  ...
  values$stationM <- "08NM116"  # multiple station list - main station 
  ...
  observeEvent(input$selectStation, {
    ...
    isolate(values$stationM <- input$station)
  })
  ...
  observeEvent(input$allstationsTable_rows_selected, {
    ...
    isolate(values$stationM <- as.data.frame(allstationsTable())[input$allstationsTable_rows_selected,1])
  })
  
  # Select station by clicking on station in map
  observeEvent(input$map_marker_click, {
    isolate(values$station  <- input$map_marker_click$id)
    isolate(values$stationM <- input$map_marker_click$id)
    updateSelectizeInput(session, "station",  selected = input$map_marker_click$id)
    updateSelectizeInput(session, "stationM", selected = input$map_marker_click$id)
  ...
  output$trendYears <- renderUI({
    sliderInput("trendYears",
                label = "Filter data between the following years:",
                min = trendDates()$minDate,
                max = trendDates()$maxDate,
                value = c(trendDates()$minDate, trendDates()$maxDate),
                sep = "")
  })
  
  output$trendMYears <- renderUI({
    sliderInput("trendMYears",
                label = "Filter data between the following years:",
                min = trendMDates()$minDate,
                max = trendMDates()$maxDate,
                value = c(trendMDates()$minDate, trendMDates()$maxDate),
                sep = "")
  })
  ...
  ## daily summaries for historic data as that needs to be filtered by years)
  ...
  source("trend-computations.R",  local=TRUE) 
  source("trendM-computations.R", local=TRUE) 
}
